...
function answerQuestion(question: string): Omit<AssistantMessage, 'from'> {
  ...
  if (/current position|today|founder|freelance|incoming software/.test(input)) return { text: jverseJourney.today, links: [{ href: '/about', label: 'Who Am I' }, { href: '/portfolio', label: 'Portfolio' }] };
  ...
  if (/technical battle|debug|scalab|integration|framework/.test(input)) return { text: jverseJourney.technical, links: [{ href: '/portfolio', label: 'Portfolio' }, { href: '/stack', label: 'Tools & Platforms' }] };
  if (/role|grind|marketer|trainer|problem solver/.test(input)) return { text: jverseJourney.roles, links: [{ href: '/about', label: 'Who Am I' }] };
  if (/journey|story|beginning|origin|vision|why.*jverse/.test(input)) return { text: jverseJourney.beginning, links: [{ href: '/about', label: 'Who Am I' }, { href: '/portfolio', label: 'Portfolio' }] };
  ...
  return { text: 'I can help you find information about JVerse, its solutions, portfolio, technology stack, collaborations, and contact options. Try asking about one of those topics.', links: [{ href: '/services', label: 'Solutions' }, { href: '/portfolio', label: 'Portfolio' }, { href: '/contact', label: 'Get in Touch' }] };
}
